...
<head>
    ...
    <link rel="shortcut icon" href="<?= BASE_PATH ?>/public/images/thumbs/<?= $pengaturan[0]['favicon'] ?>">
    <?php
...
<ul class="nav navbar-top-links navbar-right">
    <li class="dropdown">
        <a class="dropdown-toggle" data-toggle="dropdown" href="#">
            <i class="fa fa-cog fa-spin fa-1x fa-fw"></i>Setting Account <i class="fa fa-caret-down"></i>
        </a>
        <ul class="dropdown-menu dropdown-messages">
<?php
//Membuat menu user navbar
   create_menu("user/profil", "user", "User Profil");
   line();
   create_menu("user/logout", "sign-out", "Logout");
?>
        </ul>
    </li>
</ul>
...
//Membuat menu pada sidebar
   create_menu("user", "home", "Dashboard");
   create_menu("user/pemesanan", "ticket", "Pemesanan Tempat");
   create_menu("user/history","bar-chart", "History Transaksi");
   menu_drop("info","Informasi");
   menu_drop_child("user/informasi","info", "Informasi");
    close_menu();
   create_menu("user/contact","envelope", "Contact Center");
   create_menu("","home", "Beranda Website"); 
?>
        </ul>
    </div>
</div>
