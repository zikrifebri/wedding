...
class PesanController extends AdminMainController{
   ...
   public function index(){
      $query = $this->customer->selectAll();
      $data = $this->customer->getResult($query);
      $this->template('admin/pesan', 'Pesan', $data);
   }
   
   public function listData(){
      require_once ROOT."/application/functions/function_action.php";
      require_once ROOT."/application/functions/function_date.php";
      $query = $this->pesan->selectJoin(array('customer'), "LEFT JOIN", array('pesan.id_customer'=>'customer.id_customer'), "", "pesan.id_pesan", "DESC");
      ...
      foreach($list as $li){
         ...
         $row[] = $no;
         if($li['dibaca']=='N'){
            $row[] = "<b>$li[nama_lengkap] (<a href='mailto:$li[email]'>$li[email]</a>)<b>";
            $row[] = "<b>$li[subjek]<br>$tanggal<br>$li[pesan]</b>";
         }else{
            $row[] = "$li[nama_lengkap] (<a href='mailto:$li[email]'>$li[email]</a>)";
